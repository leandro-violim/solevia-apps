...
- 2026-09-03 (cont.2) — WebP everything (Cowork); pink cap wired (all caps photoreal); Home hero banner from the 3/4 render. Keeper = lime "1" cap sprite. Pitch surfaces grass/cement/street/beach/night wired as real Higgsfield textures (painted lines; wood keeps chalk); swatch previews redraw on photo decode. Ball delivered but NOT used (the cap is the ball). Surfaces confirmed **1024² webp** (~4MB decoded each — fine; my earlier 2048 note was wrong, Cowork downscaled at delivery). HUD restyled warm (cream pills). **OPEN:** optional tutorial theming; the ball has no gameplay slot. Realistic overhaul is essentially feature-complete.
