...
- 2026-09-10 — **Cowork/Higgsfield art wired in.** Cheering mascot (transparent) → Home hero + Finish + map current node (via `Mascot variant="cheer"`). Node tiles (done ★ / current pearl+flag+number / locked padlock) replace the CSS hexes in `HexNode`. The 4 world backdrops (painted bubble-wrap islands) are the `/map` per-world scenery with the 8 phase nodes overlaid at deterministic anchors (`NODE_XY`) + a soft wash for legibility; the mascot stands on the current node. Power-up bubbles (bomb/frozen/golden/mystery) now render the gamified sprite inside the clear dome (`SPECIAL_SPRITE` in Bubble.tsx; popped reuses the default). All encoded with native sharp; tsc + eslint + 39 tests green; verified Home + map in browser. **Deferred:** 1c mascot-hop animation (Cowork, optional). **Known pre-existing:** `CoinBalance`/`StreakBadge` read localStorage during render → hydration console warnings when values are non-zero (React recovers; SSR-safe refactor is a separate cleanup).
